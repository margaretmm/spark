...
object DiagModRF{
  //定义文件读出格式类  age	sex	cp	trestbps	chol	fbs	restecg	thalach	exang	oldpeak	slope	ca	thal	target
  case class Diag(
...
//    val df3 = labelIndexer.fit(df2).transform(df2)
    ...
    //val evaluator = new MulticlassClassificationEvaluator().setLabelCol("target").setPredictionCol("prediction")
    ...
    if (accuracy >0.8) {
      try {
        pipelineFittedModel.write.overwrite().save("./model/spark-LR-model-test")
